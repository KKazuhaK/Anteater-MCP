...
if (!ociPackage) throw new Error("server.json declares no oci package");
// The registry rejects a version field on an OCI package; the tag carries it.
if (ociPackage.version !== undefined) {
  throw new Error("server.json oci package must not have a version field — put the tag in identifier");
}
// Our own invariants only. The registry's rules are checked by its own validator,
// run in the release workflow — transcribing them here gave false confidence once
// already: a hand-written length check passed while the real validator rejected the
// package shape.
const ociPackageTag = ociPackage?.identifier?.split(":")[1];
if (ociPackageTag !== `v${packageJson.version}`) {
  throw new Error(
    `server.json oci identifier is tagged ${ociPackageTag || "(untagged)"}, expected v${packageJson.version}`,
  );
}
